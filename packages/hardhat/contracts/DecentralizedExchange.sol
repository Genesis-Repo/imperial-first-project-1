...
contract DecentralizedExchange {
    ...
    // An array to store all limit orders
    ...
    // Event to track the creation of a limit order
    event LimitOrderCreated(address indexed sender, address indexed token1, address indexed token2, uint256 amount, uint256 exchangeRate);
    ...
    // Function to create a new limit order
    function createLimitOrder(address _token1, address _token2, uint256 _amount, uint256 _exchangeRate) external {
        // Ensure the token pair exists
        require(isTokenPairCreated(_token1, _token2) == true, "Token pair does not exist");

        // Create a new LimitOrder struct and add it to the limitOrders array
        LimitOrder memory newLimitOrder = LimitOrder(msg.sender, _token1, _token2, _amount, _exchangeRate);
        limitOrders.push(newLimitOrder);

        emit LimitOrderCreated(msg.sender, _token1, _token2, _amount, _exchangeRate);
    }
